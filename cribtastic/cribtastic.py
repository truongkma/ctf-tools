...
# if an ASCII character was XOR'd with a space it will be a case-swapped
# ASCII character
...
def update_plaintext(x, y):
    ...
    if Key[y] == None:
        ...
        text_pt[x][y].insert(0, readable(
            chr(ord(CipherText[x][y]) ^ ord(Key[y]))))
...
# auto crack re-used one time pad by spotting where messages were XOR'd
# with a space
def otp_crack(candidates):
    global Key
    # we will process the entire list once for each ciphertext, rotating the
    # list each time so each message gets to be 'master'
    ...
    for r in range(len(candidates)):
        ...
        while other < len(candidates):
            ...
            # we nullify the key by xoring the two messages together: result is
            # m1 xor m2
            ...
            # and as we are going to repeat this process for every message, we
            # should get every key byte that corresponds to a space in any
            # message
            for x in range(len(mask)):
                if mastermask[x] != mask[x]:
                    mastermask[x] = '\0'
            other += 1
        # now we know where the spaces were in string1, an XOR with a space
        # will reveal the key byte
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "ha:b:H:l:", [
                               "help", "ascii=", "binary=", "Hexfile=", "length="])
...
widget_master = Tk()
widget_master.title(
    'Aperture Labs Ltd     cribtastic    info@aperturelabs.com')

hscrollbar = AutoScrollbar(widget_master, orient=HORIZONTAL)
vscrollbar = AutoScrollbar(widget_master)
canvas = Canvas(widget_master, yscrollcommand=vscrollbar.set,
...
# commands frame
button_command_reset = Button(
    frame_commands, text='RESET Key', command=reset_key)
button_command_reset.grid(padx=2, pady=2, row=0, column=0, sticky=W)

button_command_print = Button(
    frame_commands, text='Print Plaintexts', command=print_plaintexts)
...
button_command_auto_up = Button(
    frame_commands, text='+1', command=key_increment)
button_command_auto_up.grid(padx=2, pady=2, row=0, column=3, sticky=W)

button_command_auto_down = Button(
    frame_commands, text='-1', command=key_decrement)
...
for x in range(len(Key)):
    ...
    lock_key.append(Checkbutton(
        frame_key, variable=lock_key_var[x], command=update_all))
...
for x in range(len(CipherText)):
    ...
    hex_ciph_include.append(Checkbutton(
        frame_ciph, var=hex_ciph_include_var[x]))
    hex_ciph_include[x].bind('<Shift-ButtonRelease>',
                             ciph_include_toggle_handler)
